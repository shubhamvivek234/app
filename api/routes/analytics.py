...
def _split_posts_by_period(
    ...
    current_posts.sort(
        key=lambda item: _parse_platform_timestamp(item.get("timestamp")) or datetime.min.replace(tzinfo=timezone.utc),
        reverse=True,
    )
    previous_posts.sort(
        key=lambda item: _parse_platform_timestamp(item.get("timestamp")) or datetime.min.replace(tzinfo=timezone.utc),
...
def _followers_total_change_pct(account_summary: dict[str, Any]) -> float | None:
    total_followers = account_summary.get("followers_count")
    new_followers = account_summary.get("followers_growth")
    if total_followers is None or new_followers is None:
        return None
    previous_total = total_followers - new_followers
    if previous_total < 0:
        previous_total = 0
    return _percentage_change(total_followers, previous_total)


def _account_brand_health_row(
    account: dict[str, Any],
    ...
    platform = account.get("platform")
    supports = normalized_account.get("supports") or {}
    supports_engagements = any(supports.get(metric) for metric in ("likes", "comments", "shares", "views"))

    current_engagement = sum(_post_total_engagement(post) for post in current_posts)
    previous_engagement = sum(_post_total_engagement(post) for post in previous_posts)

    return {
        "id": normalized_account.get("account_id") or normalized_account.get("id"),
        "account_id": normalized_account.get("account_id") or normalized_account.get("id"),
        "platform": platform,
        "platform_username": normalized_account.get("platform_username"),
        "display_name": normalized_account.get("display_name"),
        "picture_url": normalized_account.get("picture_url"),
        "followers_total": normalized_account.get("followers_count") if supports.get("followers_total") else None,
        "followers_total_change_pct": _followers_total_change_pct(normalized_account) if supports.get("followers_total") else None,
        "new_followers": normalized_account.get("followers_growth") if supports.get("followers_growth") else None,
        "posts_count": len(current_posts),
        "posts_change_pct": _percentage_change(len(current_posts), len(previous_posts)),
        "reach": normalized_account.get("reach") if supports.get("reach") else None,
        "engagements": current_engagement if supports_engagements else None,
        "engagements_change_pct": _percentage_change(current_engagement, previous_engagement) if supports_engagements else None,
        "supports": supports,
        "notice": _platform_message(platform),
        "error": row_errors[0] if row_errors else None,
        "is_video_platform": platform == "youtube",
    }
...
@router.get("/analytics/brand-health")
async def analytics_brand_health(
    current_user: CurrentUser,
    db: DB,
    days: int = Query(30, ge=1, le=365),
    ...
    for account in accounts:
        platform_name = account.get("platform")
        row_errors: list[str] = []
        feed: list[dict[str, Any]] = []
        engagement: dict[str, Any] = {}

        if platform_name in _SUPPORTED_ENGAGEMENT_PLATFORMS:
            try:
                feed, engagement = await _fetch_account_feed_and_stats(db, account, days=days)
                if engagement.get("error"):
                    row_errors.append(str(engagement.get("error")))
            except Exception as exc:
                row_errors.append(_analytics_error_message(platform_name, exc))
                feed, engagement = [], {}

        if not feed:
            feed = await _fetch_db_published_posts(db, current_user["user_id"], account, limit=100)

        normalized_account = _normalize_connected_account(account, engagement)
        connected_accounts.append(normalized_account)

        current_posts, previous_posts = _split_posts_by_period(feed, current_since, previous_since)
        rows.append(
            _account_brand_health_row(
                account,
                normalized_account,
                current_posts,
                previous_posts,
        ...
        if current_posts:
            ...
            normalized_post = _normalize_feed_post(account, latest_post)
            recent_posts.append(
                {
                    **normalized_post,
                    "account_label": normalized_account.get("display_name")
                    or normalized_account.get("platform_username")
                    or account.get("platform_username")
                    or account.get("display_name")
                    or platform_name,
                    "total_engagement": _post_total_engagement(latest_post),
                    "row_error": row_errors[0] if row_errors else None,
                }
            )
