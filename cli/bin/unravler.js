...
async function handlePostsCreate(flags) {
  ...
  const scheduledTime = flags['scheduled-time'] || flags.s;
  const publishNow = Boolean(flags['publish-now'] || flags.now);
  const accountIds = flags['account-ids'] ? flags['account-ids'].split(',').map((s) => s.trim()) : [];
  const platforms = flags.platforms ? flags.platforms.split(',').map((s) => s.trim()) : [];
  const mediaUrls = flags['media-urls'] ? flags['media-urls'].split(',').map((s) => s.trim()) : [];
...
OPTIONS:
  --content, -c <text>      Post text content
  ...
  --account-ids <id1,id2>   Comma-separated target social account IDs
  --platforms <p1,p2>       Comma-separated platform names (e.g. twitter,linkedin)
  --media-urls <url1,url2>  Comma-separated media URLs
