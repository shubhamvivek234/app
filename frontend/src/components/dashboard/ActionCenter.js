      ...
      <CardHeader className="border-b border-slate-100 pb-5">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <CardTitle className="mt-2 text-2xl text-slate-950">What needs attention right now</CardTitle>
          </div>
        ...
        {!hasItems ? (
          <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-emerald-900">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-emerald-800">Connected accounts, queue, and recent activity all look healthy right now.</p>
              </div>
            </div>
          </div>
        ) : (
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
            {actionItems.map((item) => (
              <div
                key={item.id}
                className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 transition-colors hover:border-slate-300"
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div className="min-w-0">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-amber-500 shadow-sm">
                  ...
                  <Button
                    variant="outline"
                    className="shrink-0 border-slate-300 bg-white text-slate-900"
