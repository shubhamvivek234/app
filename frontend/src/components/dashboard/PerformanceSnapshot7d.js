...
const LoadingState = () => (
  <div className="space-y-6">
    <div className="rounded-2xl bg-slate-100 dark:bg-slate-800 p-5">
      ...
      <div className="mt-4 h-10 w-20 animate-pulse rounded bg-slate-200 dark:bg-slate-700" />
    </div>
    <div className="grid gap-6 lg:grid-cols-[1.3fr_1fr]">
        ...
        {[0, 1, 2].map((index) => (
          <div key={index}>
            ...
            <div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800">
              <div className="h-2 w-2/3 animate-pulse rounded-full bg-slate-200 dark:bg-slate-700" />
            </div>
          </div>
        ))}
      </div>
      <div className="space-y-6">
        <div className="grid grid-cols-3 gap-3">
          {[0, 1, 2].map((index) => (
            <div key={index} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/50">
              <div className="h-3 w-10 animate-pulse rounded bg-slate-200 dark:bg-slate-700" />
              <div className="mt-3 h-6 w-8 animate-pulse rounded bg-slate-200 dark:bg-slate-700" />
            </div>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[0, 1, 2, 3].map((index) => (
            <div key={index} className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-800/60">
              <div className="h-3 w-20 animate-pulse rounded bg-slate-200 dark:bg-slate-700" />
              <div className="mt-3 h-6 w-12 animate-pulse rounded bg-slate-200 dark:bg-slate-700" />
            </div>
          ))}
        </div>
      </div>
    ...
    <Card className="flex h-full flex-col border-slate-200 bg-white shadow-sm lg:h-[540px] dark:border-slate-800 dark:bg-slate-900">
      <CardHeader className="border-b border-slate-100 pb-5 dark:border-slate-800">
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Performance Snapshot</p>
            <CardTitle className="mt-2 text-xl text-slate-950 dark:text-slate-100">Last 7 days</CardTitle>
          </div>
          <Button variant="outline" size="sm" className="border-slate-300 bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 dark:hover:bg-slate-700" onClick={() => onNavigate?.('/analytics')}>
            Open Analytics
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-6 pt-6">
        {loading && !performance ? (
          ...
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-300">
          ...
          <>
            <div className="rounded-2xl bg-slate-950 p-5 text-white dark:border dark:border-slate-800">
            ...
            <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-[1.3fr_1fr]">
              <div className="min-h-0">
                <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Platform mix</h3>
                  ...
                  {platformEntries.length === 0 ? (
                    ...
                    <div key={platform} className="space-y-2">
                      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 text-sm">
                        <span className="truncate font-medium text-slate-700 dark:text-slate-300">{platformLabel(platform)}</span>
                        <span className="min-w-[2ch] text-right tabular-nums text-slate-500 dark:text-slate-400">{count}</span>
                      </div>
                      <div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800">
                        <div
                          className="h-2 rounded-full bg-slate-900 dark:bg-indigo-500"
              ...
              <div className="min-h-0 space-y-6 overflow-y-auto pr-1">
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Content mix</h3>
                  <div className="mt-4 space-y-3">
                    {typeEntries.length === 0 ? (
                      ...
                      <div
                        key={type}
                        className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3.5 dark:border-slate-800 dark:bg-slate-800/60"
                      >
                        ...
                        <p className="text-right text-2xl font-semibold tabular-nums text-slate-950 dark:text-slate-100">
                          {count}
                        </p>
                      </div>
                    ))}
                ...
                <div>
                  <h3 className="text-sm font-semibold uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Audience totals</h3>
                  <div className="mt-4 space-y-3">
                    {audienceMetrics.length === 0 ? (
                      ...
                      <div
                        key={label}
                        className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3.5 dark:border-slate-800 dark:bg-slate-800/50"
                      >
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-500 dark:bg-slate-700/60 dark:text-slate-400">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-1 truncate text-sm font-medium text-slate-800 dark:text-slate-200">{label}</p>
                        </div>
                        <p className="text-right text-xl font-semibold tabular-nums text-slate-950 dark:text-slate-100">
                          {compactNumber(value)}
                        </p>
                      </div>
                    ))}
