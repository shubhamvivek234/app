...
/* ─────────────────────────────────────────────────────────────────────────
   Pupil — dark dot that tracks the mouse (used for orange & yellow chars)
───────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────
   EyeBall — white circle with a tracking pupil inside (purple & black chars)
───────────────────────────────────────────────────────────────────────── */
const EyeBall = ({
...
/* ─────────────────────────────────────────────────────────────────────────
   LoginV4 — animated character mascots + clean white form
───────────────────────────────────────────────────────────────────────── */
const LoginV4 = () => {
  ...
  /* --- Typing state (email field) --- */
  const typingTimerRef = useRef(null);
  const handleEmailFocus = () => { setFocusField('email'); setIsTyping(true); };
  const handleEmailBlur  = () => { setFocusField(null); setIsTyping(false); };
  ...
  /* --- Body lean & face offset from mouse --- */
  ...
  const handleGoogleLogin = async () => {
    try { setLoading(true); await loginWithGoogle(); toast.success('Welcome back!'); }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          {/* ── Purple rectangle (behind orange) ── */}
          <div style={{
            position: 'absolute', bottom: 0, left: 148, zIndex: 1,
            width: 195, height: purpleHeight,
            background: '#6C3FF5',
            ...
            <div style={{
              ...
              transform: `translateX(${faceOffsetX * 0.3}px)`,
          ...
          {/* ── Black rectangle (right of purple) ── */}
          <div style={{
            position: 'absolute', bottom: 0, left: 308, zIndex: 2,
            width: 132, height: 318,
            background: '#2D2D2D',
            ...
            <div style={{ display: 'flex', gap: 14, marginBottom: 11 }}>
              <EyeBall size={32} pupilSize={12} maxDistance={7}
                eyeColor="white" pupilColor="#6C3FF5"
                isBlinking={isBlackBlinking}
                forceLookX={isTyping ? -10 : undefined}
                forceLookY={isTyping ? -3 : undefined}
              />
              <EyeBall size={32} pupilSize={12} maxDistance={7}
                eyeColor="white" pupilColor="#6C3FF5"
          ...
          {/* ── Yellow rounded rectangle (rightmost, in front of black) ── */}
          <div style={{
            position: 'absolute', bottom: 0, left: 415, zIndex: 3,
            width: 165, height: 248,
            background: '#E8D754',
            borderRadius: '83px 83px 0 0',
            display: 'flex', flexDirection: 'column',
            alignItems: 'center', justifyContent: 'flex-start',
            paddingTop: 42, gap: 15,
          }}>
            <div style={{ display: 'flex', gap: 20 }}>
              <Pupil size={15} maxDistance={6} pupilColor="#2D2D2D"
                forceLookX={isTyping ? -7 : undefined}
                forceLookY={isTyping ? -2 : undefined}
              />
              <Pupil size={15} maxDistance={6} pupilColor="#2D2D2D"
                forceLookX={isTyping ? -7 : undefined}
                forceLookY={isTyping ? -2 : undefined}
      ...
      {/* ══════════════════════════════════════════════════════
          RIGHT PANEL — white login form (no border / divider)
      ══════════════════════════════════════════════════════ */}
      <div style={{
        flex: '0 0 45%',
        background: '#fffffb',
        ...
        <div style={{ maxWidth: '360px', width: '100%' }}>
          ...
          <form onSubmit={handleSubmit} autoComplete="on">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '7px', cursor: 'pointer', fontSize: '13px', color: '#4b5563' }}>
                <input
                  type="checkbox"
                  checked={rememberMe}
                  onChange={(e) => setRememberMe(e.target.checked)}
                  style={{ accentColor: '#6C3FF5', width: '14px', height: '14px', cursor: 'pointer' }}
                />
                Remember for 30 days
              </label>
              <Link to="/forgot-password"
                style={{ fontSize: '13px', color: '#6C3FF5', textDecoration: 'none', fontWeight: '500' }}
          ...
          <p style={{ textAlign: 'center', fontSize: '13.5px', color: '#6b7280' }}>
            ...
            <Link to="/signup"
              style={{ color: '#6C3FF5', fontWeight: '600', textDecoration: 'none' }}
              onMouseEnter={(e) => e.target.style.textDecoration = 'underline'}
              onMouseLeave={(e) => e.target.style.textDecoration = 'none'}>
              Sign up
            </Link>
