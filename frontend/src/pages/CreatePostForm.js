...
const CreatePostForm = ({ postTypeOverride, asModal = false, onClose, editPostId: editPostIdProp = null, initialContent = '' }) => {
  ...
  const leftPanel = (
    <div className="flex-1 overflow-y-auto p-3.5 sm:p-4 md:p-5 min-w-0 bg-zinc-50/50 dark:bg-slate-950 custom-scrollbar">
      {orderedPlatforms.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="w-16 h-16 rounded-full bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 flex items-center justify-center mb-4 shadow-xs">
          ...
          <p className="text-sm font-bold text-gray-700 dark:text-slate-200">No platforms selected</p>
          <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">Select one or more accounts above to start creating your post</p>
        </div>
      ) : (
        <>
          {hasBlockingErrors && (
            <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/50 rounded-xl px-4 py-2.5 mb-3">
